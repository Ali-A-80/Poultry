...
namespace Poultry.Application.Services.Chickens
{
    public class Delete
    {
        ...
        public class ChickenDeleteValidator : AbstractValidator<Command>
        {
            public ChickenDeleteValidator()
            {
        ...
        public class Handler : IRequestHandler<Command, ResultDto<Unit>>
        {
            ...
            {
                #region Validation
                var validation = new ChickenDeleteValidator();
                ...
                var chicken = await _context.Chickens.FirstOrDefaultAsync(x => x.Id == request.Id, cancellationToken);
